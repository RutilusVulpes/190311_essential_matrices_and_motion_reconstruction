...
def main():
	...
	for x1, x2, x3 in shared_points:
		...
		X_gcp.append(triangulate(P1, P2, x1, x2))
	...
	for a, b in zip(x1_12, x2_12):
		...
		ax.scatter(vt[0], vt[1], zs=vt[2], c='blue')

	for a, b in zip(x2_23, x3_23):
		vt = triangulate(P2, P3_opt, a, b)
		ax.scatter(vt[0], vt[1], zs=vt[2], c='red')
...
def optimize_pose(shared_points, X_gcp, P2, P3):
	...
	def residual(p):
		...
		for x1, x2, x3 in shared_points:
			...
			points_23.append(triangulate(P2, P3, x2, x3))

		points_23 = np.array(points_23)

		return (X_gcp.flatten() - points_23.flatten())
...
def SIFT_stuff(I_1, I_2):
	...
	#	vt = triangulate(P_1c, P_2c, a, b)
	#	ax.scatter(vt[0], vt[1], zs=vt[2])

	#plt.show()

	return (x1[inliers, :], x2[inliers, :], P_1c, P_2c)
